...
class SiteManager
{
	...
	function LoadTemplate()
	{
		...
		if(isset($_REQUEST["page"])&&$_REQUEST["page"]!="campaign")
		{
			$this->TemplateHTML=str_replace("navbar-light fixed-top","navbar-light bg-dark fixed-top",$this->TemplateHTML);
		}
		
		if(!isset($_REQUEST["page"]) || (isset($_REQUEST["page"]) && $_REQUEST["page"]=="campaign" ) )
		{
	...
	function post_link($post_id, $post_title)
	{
		if($this->settings["website"]["seo_urls"]==1)
		{
			return "campaign-".$this->format_str(strip_tags(stripslashes($post_title)))."-".$post_id.".html";
		}
		else
		{
			return "index.php?page=campaign&id=".$post_id;
		}
	}
